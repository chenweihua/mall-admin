	...
	<div class="box col-md-12">
		<div class="box-inner">
			...
			<div class="box-content">
				...
				<label>联系人:</label><label class="labelinfo">${merchant.shopOwner}</label><br>
				<label>联系电话:</label><label class="labelinfo">${merchant.merchantName}</label><br>
				<label>收银地址:</label><label  style="word-wrap: break-word;" class="labelinfo">${merchant.mechantPayAddr}</label><br>
				<label>商户状态:</label><label class="labelinfo">
					<c:forEach var="merchantStatus" items="${MerchantStatus}">
						<c:if test="${merchant.status==merchantStatus.getType()}">
							<c:out value="${merchantStatus.name }"></c:out>
						</c:if>
					</c:forEach>
				</label><br>
