	...
	<div class="box col-md-12">
		<div class="box-inner">
			<div class="box-header well" data-original-title="">
				<h2>
					<i class="glyphicon glyphicon-user"></i> 秒杀专题商品管理
				</h2>
			...
			<div class="box-content">
					...
					<div class="col-lg-3">
						<div class="input-group">
							<span class="input-group-addon">开始时间</span> <input type="text"
								id="start-time" class="form-control" value="${beginTime}">
						</div>
					</div>
					<div class="col-lg-3">
						<div class="input-group">
							<span class="input-group-addon">结束时间</span> <input type="text"
								id="end-time" class="form-control" value="${endTime}">
						</div>
					</div>
				...
				<div class="pl15">
					...
					<button type="button" class="btn btn-primary btn-sm" id="setTimeButton">批量设置时间</button>
			...
			<div class="pl15">
				...
				<button type="button" class="btn btn-primary btn-sm" id="setTimeButton1">批量设置时间</button>
			</div>
		</div>
	</div>
	<!--/span-->
...
<!--/row-->
<%@ include file="seckillsettime.jsp"%>
...
$("#treeSubmit").click(function(){
	...
	$.post("queryActivityGoodsId", {"activityId" : activityId,"activityBgGoodsId" : activityBgGoodsId,"collegeId" : collegeId},function(ret){
		 if (ret.code != 0) {
        	...
        	setQrUrl(ret.data,1,2,collegeId, collegeName);
     		$("#modalQr").modal("show");
     	 }
	},"json");
								...
								.DataTable(
										{
											...
											searching : false,
											ajax : {
												url : "queryseckillgoods",
												data : function(d) {
													d.beginTime = $("#start-time").val();
												...
												dataSrc : function(json) {
													...
													for ( var i = 0, len = json.data.length; i < len; i++) {
														...
														newData[newData.length] = [
																...
																activityGoods.uint,
																activityGoods.showTime,
																activityGoods.beginTime,
																activityGoods.endTime,
...
/* 																'<input type="text" style="width:60px;" id="maxnum'+id+'" value="'+activityGoods.maxNum+'">', */
																...
																+ '<a class="blueletter" href="javascript:void(0)" onclick="edittime(\''+ id+'\',\''+activityGoods.beginTime+'\',\''+activityGoods.endTime+'\',\''+activityGoods.showTime+ '\')">【编辑时间】</a><br>'
...
/* 									       		var v = $('#skuList').find('tr').eq(1).find('td').eq(7).find("input").eq(0).val();
									       		$('input:radio[name="statusRadio1"][value="'+v+'"]').prop("checked",true); */
									       		
									       		var child = $('#seckillgoods').find('tr').eq(1).find('td');
									       		var id = child.eq(0).find("input").eq(0).val();
												var flag = child.eq(9).find("input").eq(0).val();
	...
	*/
	...
	function dosave(idstr){
		...
		for(var i=0;i<idarr.length;i++){
			...
			if(skuInfo.length==0){
				info.push({'activityBgGoodsId':id,'weight':weight,'status':status});
			}else{
				info.push({'activityBgGoodsId':id,'weight':weight,'status':status,'modifySkuList':skuInfo});
			}
		}
		...
		$.post("modify",
			{"modifyStr":modifyStr,"modifytype":1},
			function(response) {
