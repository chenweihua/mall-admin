...
import com.mall.admin.service.taobao.api.internal.parser.json.ObjectJsonParser;
import com.mall.admin.service.taobao.api.internal.parser.xml.ObjectXmlParser;
import com.mall.admin.service.taobao.api.internal.util.RequestParametersHolder;
import com.mall.admin.service.taobao.api.internal.util.TaobaoHashMap;
import com.mall.admin.service.taobao.api.internal.util.TaobaoLogger;
import com.mall.admin.service.taobao.api.internal.util.TaobaoUtils;
...
public class DefaultTaobaoClient implements TaobaoClient {
	...
	private int connectTimeout = 3000; // 默认连接超时时间
	private int readTimeout = 15000; // 默认响应超时时间
	...
	private boolean useGzipEncoding = true; // 是否启用响应GZIP压缩
	...
	private <T extends TaobaoResponse> T _execute(TaobaoRequest<T> request, TaobaoParser<T> parser, String session)
			throws ApiException {
	...
	public <T extends TaobaoResponse> RequestParametersHolder doPost(TaobaoRequest<T> request, String session)
			throws ApiException {
		...
		protocalOptParams.put(PARTNER_ID, getSdkVersion());
		...
		StringBuffer reqUrl = new StringBuffer(
				getServerUrl(this.serverUrl, request.getApiMethodName(), session));
		try {
		...
		try {
			...
			if (request instanceof TaobaoUploadRequest) {
				...
				rsp = WebUtils.doPost(reqUrl.toString(), appParams, fileParams, Constants.CHARSET_UTF8,
