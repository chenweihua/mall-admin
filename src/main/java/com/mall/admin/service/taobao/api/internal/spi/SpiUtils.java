package com.mall.admin.service.taobao.api.internal.spi;
...
import com.mall.admin.service.taobao.api.internal.util.StringUtils;
...
public class SpiUtils {
	 ...
	 * @return true：校验通过；false：校验不通过
	 */
	public static boolean checkSign4TextRequest(HttpServletRequest request, String body, String secret)
			throws IOException {
		String charset = WebUtils.getResponseCharset(request.getContentType());
	 ...
	 *                请求对象
	 * @param form
	 *                除了文件参数以外的所有普通文本参数的map集合
	 * @param secret
	 *                app对应的secret
	 * @return true：校验通过；false：校验不通过
	 */
	public static boolean checkSign4FileRequest(HttpServletRequest request, Map<String, String> form, String secret)
			throws IOException {
		String charset = WebUtils.getResponseCharset(request.getContentType());
		return checkSign(request, form, null, secret, charset);
	}

	private static boolean checkSign(HttpServletRequest request, Map<String, String> form, String body,
		...
		if (localSign.equals(remoteSign)) {
			...
			log.error("checkTopSign error^_^remoteSign=" + remoteSign + "^_^localSign=" + localSign
					+ "^_^paramStr=" + paramStr + "^_^body=" + body);
			return false;
		}
	}
	...
	/**
	 * 签名规则：hex(md5(secret+sorted(header_params+url_params+form_params)+body
	 * +secret)
	 */
